...
const DEFAULT_AI_WEBHOOK = "https://n8n-n8n.3asuar.easypanel.host/webhook/generar-pistas";
...
function updateHintVisibility(){
  ...
  const show = isOfficialList(listObj);
...
function prepareGame(){
  ...
  const finalHintMode = official ? hintMode : "off";
...
function showRevealForPlayer(playerNum){
  ...
  let extraHtml = "";
  if(isImp && game.listOfficial && hintMode !== "off"){
    const hint = getOfficialHint(game.secretWord, hintMode);
    if(hint){
      extraHtml = `<strong>Pista:</strong> ${escapeHtml(hint)}.`;
    }else{
...
$("pickList").addEventListener("change", ()=>{ updateHintVisibility(); });
...
// IA webhook input
if($("aiWebhook")){
  // Prefill + persist default webhook (so the button works out-of-the-box)
  if(!localStorage.getItem(LS_AI_WEBHOOK)) setAiWebhookUrl(DEFAULT_AI_WEBHOOK);
  $("aiWebhook").value = getAiWebhookUrl();
  $("aiWebhook").addEventListener("change", ()=>{
    setAiWebhookUrl($("aiWebhook").value);
    const st = $("aiStatus");
    if(st) st.textContent = getAiWebhookUrl() ? "Webhook guardado ✅" : "";
  });
}
...
async function onGenerateHintsClick(){
  ...
  const url = getAiWebhookUrl();
  if(!url){
    alert("Pega primero tu webhook de n8n en 'Webhook n8n (IA)'.");
    return;
  }
  ...
  try{
    ...
    const map = new Map(items.map(x => [String(x.word||"").trim(), x]));
...
$("btnSaveList").addEventListener("click", ()=>{
  ...
  if(selected){
    ...
    if(selected.aiHints && typeof selected.aiHints === "object"){
      const keep = {};
      for(const w of words){
        if(selected.aiHints[w]) keep[w] = selected.aiHints[w];
      }
