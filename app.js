...
function prepareGame(){
  ...
  // Guardamos el modo elegido, pero NO dependemos de un snapshot de pistas.
  // Así evitamos fallos "a veces sí / a veces no" por estados antiguos.
  ...
  return {
    ...
    secretWord,
    impostorIdxs,
    // La pista se resolverá en el momento de revelar, leyendo la lista actual de localStorage.
    hintMode,
    listId: listObj?.id || "",
...
function showRevealForPlayer(playerNum){
  ...
  // Pistas (oficiales o IA). IMPORTANTÍSIMO:
  // no dependemos de un snapshot guardado en "game" porque eso da fallos intermitentes.
  // En su lugar, leemos la lista actual desde localStorage en el momento de revelar.
  let extraHtml = "";
  if(isImp && hintMode !== "off"){
    const listsNow = loadAllLists();
    const listNow = findListById(listsNow, game.listId) || null;
    const isOfficial = isOfficialList(listNow);
    const hasCustomHints = !!(listNow && listNow.custom === true && listNow.aiHints && typeof listNow.aiHints === "object" && Object.keys(listNow.aiHints).length);

    if(isOfficial || hasCustomHints){
      let hintText = "";
      ...
      // 2) Listas personalizadas (pistas IA guardadas)
      if(!hintText && hasCustomHints){
        const aiHints = listNow.aiHints;
        // Buscar por clave normalizada (y mantener fallback por compatibilidad)
        const entry = aiHints[normWord(game.secretWord)] || aiHints[String(game.secretWord||"")] || null;
      ...
      // 3) Fallback (si la lista tiene pista activada pero falta esa palabra)
      if(hintText){
